#!/bin/bash
# ============================================================
# R70 E2E — تكملة R69 على نفس الشكوى: «مليتها فين؟ في فراغات
# في الجنب وفوق وتحت» (لقطة 1920 مخططة بالأحمر).
# الجذر: .shell كان كارت عائم — max-width:1620px بmargin:0 auto
# فبيفضل (1920-1620)/2 = 150px فاضيين في الجنبين على أي شاشة أعرض
# من 1620 + margin-block:16px = 16px فوق وتحت + بوردر وراديوس.
# الإصلاح: full-bleed — max-width:none · margin:0 · height:100vh
# · بلا بوردر/راديوس (الخلفية شبه شفافة فطبقات bg-fx لسه باينة).
# + سقف ارتفاع الجرافات اترفع 460→700 عشان الشاشات الطويلة بعد
#   ما المساحة كبرت (الموبايل min-height:100vh كمان).
# + كل رجرسون R69 (التلات إصلاحات + رجرسون R68 API) + الحراس الدائمة.
# ============================================================
set -x
...
HTML=$(curl -s "$E2E_URL/")
echo "$HTML" | grep -q 'src="/app/logo.png?v=r70"'
check "الماركب: لوجو الرئيسية بالنسخة v=r69 (cache-bust)" $?
echo "$HTML" | grep -q 'src="/app/bg/home-denim.jpg?v=r70"'
check "الماركب: خلفية الدنيم بالنسخة v=r69 (cache-bust)" $?

# ---------- R69-2: الخلفية — الحراس البكسلية الدائمة (R65/R66/R67) ----------
curl -s "$E2E_URL/app/bg/home-denim.jpg?v=r70" -o "$E2E_SCRATCH/bg_r70.jpg"
BGCHK=$(python3 - "$E2E_SCRATCH/bg_r70.jpg" <<'PYEOF'
import sys
...
CSS=$(curl -s "$E2E_URL/app/app.css?v=r70")
...
DICT=$(curl -s "$E2E_URL/app/i18n_dict.js?v=r70")
...
CSSV=$(curl -s "$E2E_URL/app/app.css?v=r70")
...
PAGES_JS=$(curl -s "$E2E_URL/app/app_pages.js?v=r70")
...
ENTRIES_JS=$(curl -s "$E2E_URL/app/app_entries.js?v=r70")
...
r = post("/api/data", {"month": "2026-09", "files": ["e2e-r70-seed.xlsx"], "pack": pack})
...
agent-browser eval "document.getElementById('efExtraCount').value='2'; document.getElementById('efExtraHours').value='3'; document.getElementById('efNote').value='e2e-r70-ui'; 'fill'" 2>/dev/null
...
# نظافة رحلة المتصفح: صف e2e-r70-ui + قفل الإدخال + إزالة تفعيل البيت
BCLEAN=$(curl -s -b "$E2E_JAR" "$E2E_URL/api/entries/overtime?month=$(date +%Y-%m)" | python3 -c "
import json,sys
d=json.load(sys.stdin)
ids=[e['id'] for e in d.get('entries',[]) if e.get('note')=='e2e-r70-ui']
print(' '.join(ids))")
for id in $BCLEAN; do
  curl -s -o /dev/null -b "$E2E_JAR" -X DELETE "$E2E_URL/api/entries/overtime?id=$id"
done
