#!/bin/bash
# ============================================================
# R69 E2E — ملاحظات المالك على واجهة R68/R30 (ثلاث شكاوى):
# 1) صفحة تحليل الأداء (بيت المدير) «محتاجة تكبر وتملي الصفحة» —
#    الارتفاع كان ثابت 260px: دلوقتي ديناميكي mhChartH() بيملي
#    مساحة المحتوى (حد أدنى 240 / أقصى 460) + إعادة رسم عند تغيير
#    مقاس النافذة + البيت بيفضل 3 أعمدة لغاية 1180 (كان بيتكسر
#    عمودين + كارت يتيم على 1280-1350).
# 2) زراير يومي/أسبوعي/شهري «الكلمة بتختفي لما بدوس» — في الثيم
#    الفاتح الخط الكريمي #F2EBDD كان فوق تدرّج شبه شفاف بيورّي ورق
#    الصفحة الفاتح. دلوقتي: .on كحلي + أبيض (نفس لغة qp-btn.on).
# 3) خانة الأشخاص الإضافيين في فورم الأوفر تايم «رتبه كويس — فيه
#    مربع فوق ومربع تحت والكلام نازل لتحت» — التسمية-الجملة كانت
#    بتتكسر 4 سطور جوه عمود ضيق والتلميح معلق تحت الحدود من بره.
#    دلوقتي: مربع واحد بعنوان سطر واحد + خانتين قصيرتين + التلميح
#    جوه المربع.
# + رجرسون R68 API (الحفظ المركب والحساب) + الحراس الدائمة.
# ============================================================
set -x
...
HTML=$(curl -s "$E2E_URL/")
echo "$HTML" | grep -q 'src="/app/logo.png?v=r69"'
check "الماركب: لوجو الرئيسية بالنسخة v=r69 (cache-bust)" $?
echo "$HTML" | grep -q 'src="/app/bg/home-denim.jpg?v=r69"'
check "الماركب: خلفية الدنيم بالنسخة v=r69 (cache-bust)" $?

# ---------- R69-2: الخلفية — الحراس البكسلية الدائمة (R65/R66/R67) ----------
curl -s "$E2E_URL/app/bg/home-denim.jpg?v=r69" -o "$E2E_SCRATCH/bg_r69.jpg"
BGCHK=$(python3 - "$E2E_SCRATCH/bg_r69.jpg" <<'PYEOF'
import sys
...
CSS=$(curl -s "$E2E_URL/app/app.css?v=r69")
...
DICT=$(curl -s "$E2E_URL/app/i18n_dict.js?v=r69")
...
PAGES_JS=$(curl -s "$E2E_URL/app/app_pages.js?v=r69")
echo "$PAGES_JS" | grep -q 'function mhChartH'
check "JS R69: دالة الارتفاع الديناميكي (mhChartH) في الكود المخدم" $?
echo "$PAGES_JS" | grep -q 'Math.max(240, Math.min(460, h))'
check "JS R69: حدود الارتفاع 240-460 (قراءة + مليان من غير بذخ)" $?
...
ENTRIES_JS=$(curl -s "$E2E_URL/app/app_entries.js?v=r69")
...
r = post("/api/data", {"month": "2026-09", "files": ["e2e-r69-seed.xlsx"], "pack": pack})
...
agent-browser eval "document.getElementById('efExtraCount').value='2'; document.getElementById('efExtraHours').value='3'; document.getElementById('efNote').value='e2e-r69-ui'; 'fill'" 2>/dev/null
...
# نظافة رحلة المتصفح: صف e2e-r69-ui + قفل الإدخال + إزالة تفعيل البيت
BCLEAN=$(curl -s -b "$E2E_JAR" "$E2E_URL/api/entries/overtime?month=$(date +%Y-%m)" | python3 -c "
import json,sys
d=json.load(sys.stdin)
ids=[e['id'] for e in d.get('entries',[]) if e.get('note')=='e2e-r69-ui']
print(' '.join(ids))")
for id in $BCLEAN; do
  curl -s -o /dev/null -b "$E2E_JAR" -X DELETE "$E2E_URL/api/entries/overtime?id=$id"
done
