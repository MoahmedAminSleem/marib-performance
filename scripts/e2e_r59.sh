#!/bin/bash
# ============================================================
# R59 E2E — Refactoring (kit.js + deleteEntry) بأمر واحد متكامل
# 1) يبني الكود بعد إعادة الهيكلة
# 2) نفس قاعدة db_seeded + نفس طلبات الـ baseline بالترتيب
# 3) يقارن: 12 JSON متطابقة + 5 ملفات XLSX متطابقة بالبايت
# 4) فحوصات refactoring الخاصة:
#    - 12 سكريبت defer (kit.js أولهم) + v=r59
#    - MaribKit معرّف في الواجهة (dstamp/ensureXLSX/dlBlob/readGrid)
#    - deleteEntry: حذف سجل إنتاج + غياب + أوفر تايم + 404 + صلاحية
#    - 🔧 باج رفع تيمبلت الغياب المصلح: رفع ملف فعلي من الواجهة
...
start_server() {
  ...
  cd .next/standalone && (NODE_ENV=production PORT=3113 bun server.js > /tmp/r59_server.log 2>&1 &)
  cd /home/z/my-project/marib-performance
...
A=scripts/e2e_r59/after; mkdir -p $A
B=scripts/e2e_r59
...
curl -s -b /tmp/r59_jar.txt "http://localhost:3113/api/data" > $A/data.json
curl -s -b /tmp/r59_jar.txt "http://localhost:3113/api/manpower" > $A/manpower.json
M=$(date +%Y-%m)
curl -s -b /tmp/r59_jar.txt "http://localhost:3113/api/entries/absence?month=$M" > $A/absence.json
curl -s -b /tmp/r59_jar.txt "http://localhost:3113/api/entries/overtime?month=$M" > $A/ot.json
curl -s -b /tmp/r59_jar.txt "http://localhost:3113/api/entries/production?month=$M" > $A/prod.json
curl -s -b /tmp/r59_jar.txt "http://localhost:3113/api/perms?me=1" > $A/perms.json
curl -s -b /tmp/r59_jar.txt "http://localhost:3113/api/settings" > $A/settings.json
curl -s -b /tmp/r59_jar.txt "http://localhost:3113/api/storage" > $A/storage.json
curl -s -b /tmp/r59_jar.txt "http://localhost:3113/api/po" > $A/po.json
curl -s -b /tmp/r59_jar.txt "http://localhost:3113/api/health" | python3 -c "
import json,sys
d=json.load(sys.stdin); d.pop('boot',None)
json.dump(d,open('scripts/e2e_r59/after/health.json','w'),sort_keys=True,ensure_ascii=False)"
...
json.dump(d,open('scripts/e2e_r59/after/audit.json','w'),sort_keys=True,ensure_ascii=False)"
curl -s -b /tmp/r59_jar.txt "http://localhost:3113/api/users" | python3 -c "
import json,sys
d=json.load(sys.stdin)
for u in d.get('users',[]): u['created']='<TS>'
json.dump(d,open('scripts/e2e_r59/after/users.json','w'),sort_keys=True,ensure_ascii=False)"

curl -s -b /tmp/r59_jar.txt "http://localhost:3113/api/manpower/export" -o $A/xlsx_ar.xlsx
curl -s -b /tmp/r59_jar.txt "http://localhost:3113/api/manpower/export?lang=tr" -o $A/xlsx_tr.xlsx
curl -s -b /tmp/r59_jar.txt "http://localhost:3113/api/manpower/export?template=1" -o $A/xlsx_tpl.xlsx
curl -s -b /tmp/r59_jar.txt "http://localhost:3113/api/entries/absence?template=1" -o $A/xlsx_abs.xlsx
curl -s -b /tmp/r59_jar.txt "http://localhost:3113/api/po?template=1" -o $A/xlsx_po.xlsx
...
# سجل إنتاج تجريبي
P1=$(curl -s -b /tmp/r59_jar.txt -X POST http://localhost:3113/api/entries/production \
  -H 'Content-Type: application/json' \
  -d '{"date":"2026-09-10","dept":"الصدر","line":"2","po_number":"R59-DEL","qty":50}')
P1_ID=$(echo "$P1" | python3 -c "import json,sys;print(json.load(sys.stdin).get('id',''))" 2>/dev/null)
[ -n "$P1_ID" ]; check "سجل إنتاج تجريبي اتحفظ" $?
D1=$(curl -s -o /dev/null -w "%{http_code}" -b /tmp/r59_jar.txt -X DELETE "http://localhost:3113/api/entries/production?id=$P1_ID")
[ "$D1" = "200" ]; check "deleteEntry: حذف إنتاج = 200" $?
D2=$(curl -s -o /dev/null -w "%{http_code}" -b /tmp/r59_jar.txt -X DELETE "http://localhost:3113/api/entries/production?id=$P1_ID")
[ "$D2" = "404" ]; check "deleteEntry: حذف تاني = 404 (نفس السلوك القديم)" $?
D3=$(curl -s -o /dev/null -w "%{http_code}" -b /tmp/r59_jar.txt -X DELETE "http://localhost:3113/api/entries/production?id=")
[ "$D3" = "400" ]; check "deleteEntry: بدون id = 400" $?
# غياب + أوفر تايم (نفس الدالة المشتركة)
A1=$(curl -s -b /tmp/r59_jar.txt -X POST http://localhost:3113/api/entries/absence \
  -H 'Content-Type: application/json' -d '{"date":"2026-09-10","emp_code":"17001","reason":"r59"}')
A1_ID=$(echo "$A1" | python3 -c "import json,sys;print(json.load(sys.stdin).get('id',''))" 2>/dev/null)
DA=$(curl -s -o /dev/null -w "%{http_code}" -b /tmp/r59_jar.txt -X DELETE "http://localhost:3113/api/entries/absence?id=$A1_ID")
[ "$DA" = "200" ]; check "deleteEntry: حذف غياب = 200" $?
O1=$(curl -s -b /tmp/r59_jar.txt -X POST http://localhost:3113/api/entries/overtime \
...
DO=$(curl -s -o /dev/null -w "%{http_code}" -b /tmp/r59_jar.txt -X DELETE "http://localhost:3113/api/entries/overtime?id=$O1_ID")
[ "$DO" = "200" ]; check "deleteEntry: حذف أوفر تايم = 200" $?
# صلاحية: يوزر عادي (data.upload = hidden افتراضي) → 403
curl -s -b /tmp/r59_jar.txt -X POST http://localhost:3113/api/users \
  -H 'Content-Type: application/json' \
  -d '{"username":"TestR59","password":"test1234","admin":false}' > /dev/null
R59U=$(curl -s -b /tmp/r59_jar.txt http://localhost:3113/api/users | python3 -c "import json,sys;print([u['id'] for u in json.load(sys.stdin).get('users',[]) if u['username']=='TestR59'][0])")
curl -s -c /tmp/r59_jar2.txt -X POST http://localhost:3113/api/auth \
  -H 'Content-Type: application/json' \
  -d '{"username":"TestR59","password":"test1234","remember":true}' > /dev/null
DP=$(curl -s -o /dev/null -w "%{http_code}" -b /tmp/r59_jar2.txt -X DELETE "http://localhost:3113/api/entries/production?id=whatever")
[ "$DP" = "403" ]; check "deleteEntry: يوزر عادي = 403 (الحارس شغال من المصدر المشترك)" $?
curl -s -b /tmp/r59_jar.txt -X DELETE "http://localhost:3113/api/users?id=$R59U" > /dev/null
...
rows = [
    ...
    '<row r="2"><c r="A2" t="inlineStr"><is><t>1</t></is></c><c r="B2" t="inlineStr"><is><t>17001</t></is></c><c r="C2" t="inlineStr"><is><t>ر59 تجربة</t></is></c><c r="D2" t="inlineStr"><is><t>ازمة</t></is></c></row>',
    '<row r="3"><c r="A3" t="inlineStr"><is><t>2</t></is></c><c r="B3" t="inlineStr"><is><t>17002</t></is></c><c r="C3" t="inlineStr"><is><t>ر59 تجربة2</t></is></c><c r="D3" t="inlineStr"><is><t>مرضي</t></is></c></row>',
]
...
with zipfile.ZipFile('scripts/e2e_r59/upload_absence.xlsx', 'w', zipfile.ZIP_DEFLATED) as z:
...
agent-browser upload "input.ent-file-pick" /home/z/my-project/marib-performance/scripts/e2e_r59/upload_absence.xlsx 2>/dev/null
sleep 2
...
agent-browser screenshot scripts/e2e_r59/ui_final.png 2>/dev/null

pkill -f "bun server.js" 2>/dev/null

echo ""
echo "=========== النتيجة: PASS=$PASS FAIL=$FAIL ==========="
[ $FAIL -eq 0 ] && echo "E2E R59: كل الفحوصات نجحت" || { echo "E2E R59: فيه فشل — راجع فوق"; exit 1; }
